...
class HttpExceptionRenderer extends AbstractErrorRenderer
{
    ...
    {
        if ($exception instanceof \MatDave\ActivityPub\Api\Exceptions\RestfulException) {
    ...
    /**
     * @param \MatDave\ActivityPub\Api\Exceptions\RestfulException  $exception
     *
     * @return string
     */
    private function formatExceptionFragment(\MatDave\ActivityPub\Api\Exceptions\RestfulException $exception): string
    {
        $text = sprintf("Type: %s\n", get_class($exception));
